...
(§5, nine models, paired three-seed A/B): it does not move `usage` macro-F1, and
...
which is what §5 then runs.

## 5. Measured, not predicted — we ran it

§4 argued the effect would be unmeasurable. That is a prediction, so it was tested:
...
domain gap §3(d) measured at AUC 0.978 — now confirmed by outcome rather than by
proxy.
...
3. **Do not train the submitted model on this set.** Not on the §3 ceiling argument
   alone — on the §5 measurement: nine trained models, `Party` F1 0.0000 in every
...
5. **If more `usage` data is still wanted**, the only classes worth the effort are
   ...
   to match the provided domain. But note §3(c): the provided data already contains
