...
| Recorded size | 126-256 px, mean 237 x 224 |
...
- **Near-duplicate** (16x16 normalised-thumbnail cosine + 64-bit dHash): highest cosine to
...
Adversarial validation after the pipeline's own preprocessing (`ImageOps.pad` to 60x80):
...
full-body model photos, and tested whether the gap was an artefact of padding square 256x256
...
With no `labels=`, sklearn averages over the classes present in `y_true or y_pred` only.
